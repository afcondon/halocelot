...
_a = SProxy :: SProxy "a" -- look at putting proper labels later
_b = SProxy :: SProxy "b"
_c = SProxy :: SProxy "c"
_d = SProxy :: SProxy "d"
_e = SProxy :: SProxy "e"
  ...
  where
    ...
    eval (Initialize next) = do
      ...
      _ <- case selectedLocations of
        Success xs -> do
          _ <- H.query' _c 1 $ TA.ReplaceSelected (head xs) unit
          _ <- H.query' _d 2 $ TA.ReplaceSelected (take 4 xs) unit
          _ <- H.query' _c 5 $ TA.ReplaceSelected (head xs) unit
          pure unit
        otherwise -> pure unit
      ...
      _ <- case selectedUsers of
        Success xs -> do
          _ <- H.query' _a 1 $ TA.ReplaceSelected (head xs) unit
          _ <- H.query' _b 3 $ TA.ReplaceSelected (take 4 xs) unit
          _ <- H.query' _b 5 $ TA.ReplaceSelected (take 4 xs) unit
          pure next
        otherwise -> pure next

      _ <- H.query' _a 6 $ H.action $ TA.ReplaceItems $ Failure ""
      _ <- H.query' _a 7 $ H.action $ TA.ReplaceItems Loading
